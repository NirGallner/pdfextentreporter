...
public class Annotation {
	...
	public static void updateDestination(/* ReportData reportData */) {
		 ...
		 * feature.getAnnotations().forEach(a -> { PDActionGoTo action = new
		 * PDActionGoTo(); action.setDestination(feature.getDestination());
		 * a.setAction(action); });
		 * 
		 * for (Scenario scenario : feature.getScenarios()) {
		 * scenario.getAnnotations().forEach(a -> { PDActionGoTo action = new
		 * PDActionGoTo(); action.setDestination(scenario.getDestination());
		 * a.setAction(action); }); } }
		 */
	}
}
